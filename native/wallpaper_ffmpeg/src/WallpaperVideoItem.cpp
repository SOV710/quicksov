...
QSGNode *WallpaperVideoItem::updatePaintNode(QSGNode *oldNode, UpdatePaintNodeData *data) {
    ...
    if (m_texture == nullptr || m_textureSerial != snapshot.serial) {
        clearTexture();
        m_texture = window()->createTextureFromImage(snapshot.image);
...
QSize WallpaperVideoItem::pixelSizeHint() const {
    ...
    const qreal dpr = itemWindow->effectiveDevicePixelRatio();
    return QSize(
