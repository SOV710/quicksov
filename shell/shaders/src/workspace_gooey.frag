...
void main() {
    ...
    d = smin(d, sdCircle(px, source, sourceRadius), k * sourceKeep);
    ...
    d = smin(d, sdCapsule(px, source, moving, sourceBridgeRadius), k * tail);

    float targetBridgeRadius = r * 0.48 * tail * smoothstep(0.42, 1.0, p);
    d = smin(d, sdCapsule(px, moving, target, targetBridgeRadius), k * tail);
