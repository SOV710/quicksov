...
| 参数 | 值 |
|---|---|
| `group_container_height` | 32-33px |
| `group_container_pad_x` | 8px |
| `group_container_radius` | 16px（= `radius.md`） |
| `leaf_chip_height` | 16-24px（视组件而定；workspace spot 优先 16px） |
| `leaf_chip_radius` | 12px（= `radius.sm`） |
| `status_capsule_height` | 39-40px |
| `status_capsule_radius` | 19-20px（= `radius.lg`） |
...
| 参数 | 值 |
|---|---|
| `clock_segment_height` | 32px |
| `clock_segment_outer_radius` | 16px |
| `clock_segment_gap` | 0px |
| `clock_segment_min_width` | 52-56px |
