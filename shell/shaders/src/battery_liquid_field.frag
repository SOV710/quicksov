...
void main() {
    ...
    float theta = ubuf.phase * 6.28318530718;
    ...
    float carrier = ubuf.waveAmplitude * (
        0.52 * sin(y * 8.0 + theta) +
        0.28 * sin(y * 16.0 - theta * 2.0) +
        0.20 * sin((uv.x * 2.0 + y * 6.0) * 4.0 + theta * 3.0)
    );
    float breathing = edgeSoftness * 0.26 * sin(theta + y * 1.4);
    ...
    float shimmer = 0.5 + 0.5 * sin(theta * 3.0 + y * 26.0 + uv.x * 4.0);
